...
export const auth = betterAuth({
  ...
  plugins: [
    ...
    customSession(async ({ user, session }) => {
      ...
      // Exclude password from response - never send password to client
  ...
  events: {
    onUserCreate: async ({ user }: { user: any }) => {
      ...
      try {
        ...
        // Fetch full user data including password for console logging
        const fullUserData = await prisma.user.findUnique({
          where: { id: user.id },
        });

        // Console log full user data with all fields including password
        console.log("\n" + "=".repeat(80));
        console.log("📋 FULL USER DATA (ALL FIELDS INCLUDING PASSWORD)");
